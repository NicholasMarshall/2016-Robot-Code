...
public class FastGyroTurn extends Command {
	
	double Kp = 0.0;
	double Ki = 0.0;
	double angle = 0.0;
	
    public FastGyroTurn(double angle) {
        ...
        this.angle = RexRobot.navx.getYaw() + angle;
    }
    ...
    protected void execute() {
    	if(counter == 0) {
	    	...
	    	if(Math.abs(delta) > 30.0) {
	    		double output = 400 * Math.signum(delta);
	    	...
	    	} else if(Math.abs(delta) > 10.0 && Math.abs(delta) < 30.0) {
	    		double output = 150 * Math.signum(delta);
		    	RexRobot.frontLeftSwerveWheel.setDrive(0.0);
		    	RexRobot.frontRightSwerveWheel.setDrive(0.0);
		    	RexRobot.rearLeftSwerveWheel.setDrive(output);
		    	RexRobot.rearRightSwerveWheel.setDrive(output);
	    	} else if(Math.abs(delta) > 1.0 && Math.abs(delta) < 10.0) {
	    		double output = 50 * Math.signum(delta);
	    		RexRobot.frontLeftSwerveWheel.setDrive(0.0);
		    	RexRobot.frontRightSwerveWheel.setDrive(0.0);
		    	RexRobot.rearLeftSwerveWheel.setDrive(output);
		    	RexRobot.rearRightSwerveWheel.setDrive(output);
