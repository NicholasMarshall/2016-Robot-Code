...
public class FollowSpeedProfile extends Command {
    ...
    public FollowSpeedProfile(double length, double value2, double time2, double value3, double time3, boolean curve) {
    ...
    public FollowSpeedProfile(double length, double value2, double time2, double value3, double time3, boolean curve, boolean reverse) {
    	requires(RexRobot.frontLeftSwerveWheel);
        requires(RexRobot.frontRightSwerveWheel);
        requires(RexRobot.rearLeftSwerveWheel);
        requires(RexRobot.rearRightSwerveWheel);
    ...
    protected void execute() {
    	if(curve) {
    		...
    		double add = Kp * angle;
	    	RexRobot.frontLeftSwerveWheel.setDrive((profile.getValue(timer.get()) - add) * multiple);
